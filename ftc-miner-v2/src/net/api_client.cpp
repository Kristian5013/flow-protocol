...
// Helper: connect to host:port with IPv4/IPv6 support
...
APIClient::NetworkStats APIClient::getNetworkStats() {
    ...
    if (!status_response.empty()) {
        ...
        if (peers_pos != std::string::npos) {
            size_t connected_pos = status_response.find("\"connected\":", peers_pos);
            if (connected_pos != std::string::npos) {
                stats.peer_count = std::stoul(status_response.substr(connected_pos + 12));
            }
        }
    }
