 ...
 *   ftc-wallet balance <address>     Check balance
...
void cmdBalance(const std::string& address, const std::string& node = "127.0.0.1:17319") {
    size_t colon = node.find(':');
    std::string host = (colon != std::string::npos) ? node.substr(0, colon) : node;
    int port = (colon != std::string::npos) ? std::stoi(node.substr(colon + 1)) : 17319;
...
void cmdSend(const std::string& privkey_hex, const std::string& to_address,
             const std::string& amount_str, const std::string& node = "127.0.0.1:17319") {
    secp256k1::init();
...
int main(int argc, char** argv) {
    ...
    if (cmd == "new" || cmd == "generate") {
        ...
        std::string node = (argc >= 4) ? argv[3] : "127.0.0.1:17319";
        ...
        std::string node = (argc >= 6) ? argv[5] : "127.0.0.1:17319";
