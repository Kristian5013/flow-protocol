...
std::shared_ptr<Connection> Connection::connect(const NetAddr& addr, ConnectCallback on_connect) {
    // Create socket
    int family = addr.isIPv4() ? AF_INET : AF_INET6;
...
bool Listener::bind(uint16_t port, bool ipv6) {
    ...
    sock_ = socket(family, SOCK_STREAM, IPPROTO_TCP);
    ...
    if (ipv6) {
        ...
        int off = 0;
        setsockopt(sock_, IPPROTO_IPV6, IPV6_V6ONLY, (char*)&off, sizeof(off));

        result = ::bind(sock_, (struct sockaddr*)&sa, sizeof(sa));
    } else {
        struct sockaddr_in sa;
        memset(&sa, 0, sizeof(sa));
        sa.sin_family = AF_INET;
...
std::shared_ptr<Connection> Listener::accept() {
    ...
    if (client_addr.ss_family == AF_INET) {
        ...
        addr = NetAddr::fromIPv4(ip, ntohs(sa->sin_port));
    } else if (client_addr.ss_family == AF_INET6) {
        struct sockaddr_in6* sa = (struct sockaddr_in6*)&client_addr;
        addr = NetAddr::fromIPv6((uint8_t*)&sa->sin6_addr, ntohs(sa->sin6_port));
    }
