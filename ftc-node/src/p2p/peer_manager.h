...
public:
    ...
    size_t getPeerCount() const;       // Unique nodes by IP
    size_t getConnectionCount() const;  // Total connections
...
private:
    ...
    // Local addresses (to avoid self-connection)
    std::set<std::array<uint8_t, 16>> local_ips_;
