...
Config Config::parse(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg.rfind("--addnode=", 0) == 0) {
            // --addnode=IP:PORT format
            config.addnodes.push_back(arg.substr(10));
        }
        else if (arg == "--addnode") {
            // --addnode IP:PORT format
            if (i + 1 < argc) {
...
void Config::printHelp() {
    ...
    std::cout << "  --addnode=IP:PORT       Connect to peer (can specify multiple times)\n";
    ...
    std::cout << "\n";
    std::cout << "Examples:\n";
    std::cout << "  First node (no peers):     ftc-node\n";
    std::cout << "  Connect to peer:           ftc-node --addnode=192.168.1.10:17318\n";
    std::cout << "  Multiple peers:            ftc-node --addnode=1.2.3.4:17318 --addnode=5.6.7.8:17318\n";
    std::cout << "  IPv6:                      ftc-node --addnode=[2001:db8::1]:17318\n";
    std::cout << "\n";
}
