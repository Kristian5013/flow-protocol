 ...
 *
 * Peer discovery:
 * - First node starts without peers (just listens)
 * - Other nodes use --addnode=IP:PORT to connect
 * - Peers exchange addresses via ADDR/GETADDR
 * - peers.dat persists known addresses
 */
class Node {
public:
...
private:
    ...
    // Connect to initial --addnode peers
